...
> for integration into the Fabric AgentHub custom workload.
...
|---|---|---|
| UI Framework | React + FluentUI v9 | Matches AgentHub Frontend stack |
| API Layer | Fabric REST API + PBI REST API | Python used sempy_labs (XMLA/TOM) — REST is browser-compatible |
| Auth | Token passed as prop | AgentHub handles OAuth, passes token down |
