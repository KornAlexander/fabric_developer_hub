...
export const ChatPanel: React.FC<ChatPanelProps> = ({ isOpen, onClose, workloadContext, workloadClient }) => {
    ...
    const sendMessage = useCallback(async () => {
        ...
        if (workloadContext) {
            systemMessages.push({
                role: 'system',
                content: `You are a helpful assistant integrated into a Microsoft Fabric workload called AgentHub. ${workloadContext}`,
            });
        }
        ...
        <div className="chat-panel">
            ...
            <div className="chat-panel-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <Text weight="semibold" size={400}>AgentHub Chat</Text>
                </div>
                        ...
                        <>
                            <Text size={300} align="center">
                                ...
                                <a
                                    ...
                                    onClick={async (e) => {
                                        e.preventDefault();
                                        const url = `${deviceFlow.verificationUri}?user_code=${deviceFlow.userCode}`;
                                        try {
                                            if (workloadClient) {
                                                await workloadClient.navigation.openBrowserTab({ url, queryParams: {} });
                                            } else {
