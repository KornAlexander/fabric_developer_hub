...
from models.onelake_folder import OneLakePathContainer
...
class LakehouseClientService:
    ...
    async def get_lakehouse_tables(self, token: str, workspace_id: UUID, lakehouse_id: UUID) -> list[LakehouseTable]:
        ...
        filtered_paths = [
            ...
            if path.name.endswith(delta_log_directory) or
               (path.is_shortcut and path.account_type == "ADLS")
        ]
    ...
    async def get_fabric_lakehouse(self, token: str, workspace_id: UUID, lakehouse_id: UUID) -> FabricItem | None:
        """
