...
AgentHub is a Microsoft Fabric workload that hosts conversational agents inside Fabric
...
- `Backend/python/` — FastAPI service implementing the Fabric workload contract. Uses
...
`Backend/python/src/Packages/manifest/WorkloadManifest.xml` and `Item1.xml` are
templates — `${WORKLOAD_NAME}`, `${CLIENT_ID}`, and `${AUDIENCE}` are substituted
from `AgentHub/.env` at build time by `tools/manifest_package_generator.py`. Each
