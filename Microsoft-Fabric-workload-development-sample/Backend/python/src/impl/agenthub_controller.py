...
async def _copilot_token(request: Request) -> str:
    ...
    from impl.github_chat_controller import _extract_github_token, _get_copilot_token
...
async def _mcp_tokens(request: Request) -> Optional[dict]:
    ...
    if not fabric_token:
        return None
    from impl.github_chat_controller import _acquire_mcp_tokens
...
async def create_job(req: CreateJobRequest, request: Request):
    ...
    plan = await orchestrator_engine.generate_plan(
...
async def send_message_to_job(job_id: str, req: SendMessageRequest):
    ok = orchestrator_engine.inject_message(job_id, req.message, req.target_agent_id)
    if not ok:
...
async def job_events_sse(job_id: str):
    ...
    execution = orchestrator_engine.get_job_execution(job_id)
    if not execution:
...
async def approve_plan(req: ApprovePlanRequest, request: Request):
    ...
    await orchestrator_engine.start_job(job, copilot_token, mcp_tokens)
...
async def delete_my_agent(config_id: str):
    ok = job_store.delete_agent_config(config_id)
    if not ok:
...
async def get_audit(job_id: str):
    return job_store.get_audit_log(job_id)
