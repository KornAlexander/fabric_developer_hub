...
class TestErrorRecoveryScenarios:
    ...
    async def test_partial_service_degradation(self, auth_fixtures):
        ...
        with patch("services.auth.authentication.get_configuration_service", return_value=mock_config):
    ...
    async def test_network_timeout_resilience(self, auth_fixtures):
        ...
        service.openid_manager.get_configuration_async.side_effect = TimeoutError("Network timeout")
    ...
    async def test_jwt_validation_external_dependency_failures(self, auth_fixtures):
        ...
        auth_fixtures.create_jwt_payload(tenant_id="test-tenant", token_version="2.0")
        ...
        with patch.object(service.openid_manager, 'get_configuration_async', return_value=mock_config):
            ...
            with patch('services.auth.authentication.jwt.get_unverified_header', side_effect=Exception("JWT library error")):
                with pytest.raises(AuthenticationException, match="Token validation failed"):
                    await service._validate_aad_token_common(token, False, None)

            # Test JWT claims extraction failure
            with patch('services.auth.authentication.jwt.get_unverified_header', return_value={"kid": "test-key-id"}):
                with patch('services.auth.authentication.jwt.get_unverified_claims', side_effect=Exception("Claims extraction failed")):
                    with pytest.raises(AuthenticationException, match="Token validation failed"):
                        await service._validate_aad_token_common(token, False, None)
    ...
    async def test_concurrent_token_validation(self, auth_fixtures):
        ...
        with patch('services.auth.authentication.jwt.get_unverified_header', return_value={"kid": "test-key-id"}):
            with patch('services.auth.authentication.jwt.get_unverified_claims', side_effect=[payload1, payload2]):
                with patch('services.auth.authentication.jwt.decode', side_effect=[payload1, payload2]):
                    with patch.object(service.openid_manager, 'get_configuration_async', return_value=mock_config):
    ...
    async def test_service_initialization_under_load(self, auth_fixtures):
        ...
        async def create_service():
            ...
            with patch("services.auth.authentication.get_configuration_service", return_value=mock_config_service):
                with patch("services.auth.authentication.msal") as mock_msal:
                    mock_app = Mock()
