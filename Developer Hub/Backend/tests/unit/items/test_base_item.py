...
class TestItemBase:
    ...
    def test_init_creates_required_services(self, auth_context, mock_item_metadata_store,
        ...
        with patch('services.fabric.item_metadata_store.get_item_metadata_store', return_value=mock_item_metadata_store), \
            ...
            expected_logger_name = f"domain.items.base_item.{ConcreteTestItem.__name__}"
    ...
    def test_logger_initialization(self, auth_context):
        ...
        with patch('services.fabric.item_metadata_store.get_item_metadata_store'), \
            ...
            expected_name = f"domain.items.base_item.{item.__class__.__name__}"
    ...
    async def test_cancel_job_sets_canceled_time(self, auth_context, mock_item_metadata_store,
        ...
        with patch('services.fabric.item_metadata_store.get_item_metadata_store', return_value=mock_item_metadata_store), \
            ...
            with patch('datetime.datetime') as mock_datetime:
                mock_utc_time = datetime.datetime(2023, 7, 16, 12, 30, 45, tzinfo=datetime.UTC)
                mock_datetime.now.return_value = mock_utc_time
                mock_datetime.timezone.utc = datetime.UTC
