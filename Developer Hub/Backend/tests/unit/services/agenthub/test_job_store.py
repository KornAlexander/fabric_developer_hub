"""End-to-end CRUD tests for ``services.agenthub.job_store`` against a
...
def _isolated_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(job_store, "_DB_PATH", None)
    job_store.init_db()
    yield
    monkeypatch.setattr(job_store, "_DB_PATH", None)
...
def test_get_job_returns_none_for_missing_id() -> None:
    assert job_store.get_job("nope") is None


def test_list_jobs_filters_by_user_and_status() -> None:
    job_store.create_job(_make_job("a", "alice", JobStatus.PLANNED))
    job_store.create_job(_make_job("b", "alice", JobStatus.RUNNING))
    job_store.create_job(_make_job("c", "bob", JobStatus.RUNNING))

    alice_jobs = job_store.list_jobs("alice")
    assert {j.id for j in alice_jobs} == {"a", "b"}

    alice_running = job_store.list_jobs("alice", status="running")
    assert {j.id for j in alice_running} == {"b"}

    bob_jobs = job_store.list_jobs("bob")
...
def test_list_jobs_orders_by_created_at_desc() -> None:
    ...
    job_store.create_job(older)
    job_store.create_job(newer)

    listed = job_store.list_jobs("u")
    assert [j.id for j in listed] == ["new", "old"]


def test_list_jobs_respects_limit() -> None:
    for i in range(5):
        job_store.create_job(_make_job(f"j{i}", "u"))
    assert len(job_store.list_jobs("u", limit=3)) == 3
...
def test_delete_job_returns_true_when_deleted_false_when_missing() -> None:
    job_store.create_job(_make_job("x"))
    assert job_store.delete_job("x") is True
    assert job_store.get_job("x") is None
    assert job_store.delete_job("x") is False
...
def test_save_and_get_user_agent_configs() -> None:
    job_store.save_agent_config(_make_config("c1"))
    job_store.save_agent_config(_make_config("c2"))
    job_store.save_agent_config(_make_config("c3", user_id="other"))

    configs = job_store.get_user_agent_configs("u-1")
...
def test_save_agent_config_replaces_on_same_id() -> None:
    ...
    cfg = _make_config()
    job_store.save_agent_config(cfg)
...
def test_delete_agent_config_returns_true_only_when_existed() -> None:
    job_store.save_agent_config(_make_config("c-del"))
    assert job_store.delete_agent_config("c-del") is True
    assert job_store.delete_agent_config("c-del") is False
...
def test_log_audit_and_retrieve_in_chronological_order() -> None:
    ...
    job_store.log_audit("j-2", None, None, None, "unrelated")

    rows = job_store.get_audit_log("j-1")
    assert len(rows) == 2
...
def test_get_audit_log_empty_when_no_entries() -> None:
    assert job_store.get_audit_log("never-logged") == []
