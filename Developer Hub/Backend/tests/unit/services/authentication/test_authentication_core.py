...
class TestTokenValidation:
    ...
    async def test_validate_aad_token_common_success(self, auth_fixtures):
        ...
        with patch('services.auth.authentication.jwt.get_unverified_header', return_value={"kid": "test-key-id"}):
            with patch('services.auth.authentication.jwt.get_unverified_claims', return_value=payload):
                with patch('services.auth.authentication.jwt.decode', return_value=payload):
    ...
    async def test_validate_aad_token_expired(self, auth_fixtures):
        ...
        with patch('services.auth.authentication.jwt.get_unverified_header', return_value={"kid": "test-key-id"}):
            with patch('services.auth.authentication.jwt.get_unverified_claims', return_value=payload):
                with patch('services.auth.authentication.jwt.decode', side_effect=ExpiredSignatureError("Token expired")):
                    with patch.object(service.openid_manager, 'get_configuration_async', return_value=mock_config):
...
class TestOBOFlow:
    ...
    async def test_obo_flow_ui_required(self, auth_fixtures):
        ...
        with patch.object(service, '_get_msal_app', return_value=mock_app):
            with patch('services.auth.authentication.AuthenticationUIRequiredException') as mock_ui_ex:
