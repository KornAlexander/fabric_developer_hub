...
class TestMSALErrorHandlingParameterized:
    ...
    async def test_msal_client_not_configured_scenarios(self, auth_fixtures):
        ...
        with patch("services.auth.authentication.get_configuration_service", return_value=mock_config_no_id):
        ...
        with patch("services.auth.authentication.get_configuration_service", return_value=mock_config_no_secret):
            service = AuthenticationService(openid_manager=mock_openid_manager)
            auth_context = auth_fixtures.create_auth_context()
...
class TestMSALAppManagement:
    ...
    def test_msal_app_cache_isolation(self, auth_fixtures):
        ...
        with patch("services.auth.authentication.msal") as mock_msal:
            ...
            mock_apps = []
            for _i, tenant in enumerate(test_tenants):
                mock_app = Mock()
