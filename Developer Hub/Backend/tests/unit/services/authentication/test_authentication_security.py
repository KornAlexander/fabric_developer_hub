...
from domain.models.authentication_models import Claim
...
class TestTokenSecurityValidation:
    ...
    async def test_token_signature_validation_security(self, auth_fixtures):
        ...
        with patch('services.auth.authentication.jwt.get_unverified_header', return_value={"kid": "unknown-key"}):
    ...
    async def test_token_audience_validation_security(self, auth_fixtures):
        ...
        with patch('services.auth.authentication.jwt.get_unverified_header', return_value={"kid": "test-key-id"}):
            with patch('services.auth.authentication.jwt.get_unverified_claims', return_value=payload):
                with patch('services.auth.authentication.jwt.decode', side_effect=JWTClaimsError("Invalid audience")):
                    with patch.object(service.openid_manager, 'get_configuration_async', return_value=mock_config):
    ...
    async def test_malformed_token_security(self, auth_fixtures):
        ...
        for malformed_token in malformed_tokens:
            with patch('services.auth.authentication.jwt.get_unverified_header', side_effect=Exception("Invalid token")):
