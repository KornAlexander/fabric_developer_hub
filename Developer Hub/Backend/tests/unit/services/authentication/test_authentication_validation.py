...
from domain.models.authentication_models import Claim, TokenVersion
...
class TestConfigurationHandling:
    ...
    def test_missing_configuration_scenarios(self, auth_fixtures):
        ...
        with patch("services.auth.authentication.get_configuration_service", return_value=mock_config_all_none):
        ...
        with patch("services.auth.authentication.get_configuration_service", return_value=mock_config_partial):
            service = AuthenticationService(openid_manager=mock_openid_manager)
            assert len(service._msal_apps) == 0
        ...
        with patch("services.auth.authentication.get_configuration_service", return_value=mock_config_empty):
            service = AuthenticationService(openid_manager=mock_openid_manager)
